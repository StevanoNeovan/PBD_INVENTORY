...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="mb-0"><i class="fas fa-ruler"></i> Master Vendor</h2>
            <p class="text-muted mb-0">Kelola vendor</p>
        </div>
    ...
    <div class="card shadow">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0"><i class="fas fa-list"></i> Daftar Vendor</h5>
        </div>
        <div class="card-body">
            <!-- Filter Status -->
            <div class="mb-3">
                <div class="btn-group" role="group">
                    <a href="{{ route('superadmin.vendor.index') }}" 
            ...
            <div class="table-responsive">
                <table class="table table-hover table-bordered" id="tableVendor">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-center">{{ $item->idvendor }}</td>
                            <td><strong>{{ $item->nama_vendor }}</strong></td>
                            <td class="text-center">
                                @if($item->status_vendor == 'Aktif')
                            ...
                            <td class="text-center">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('superadmin.vendor.edit', $item->idvendor) }}" 
                                       class="btn btn-sm btn-warning">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    
                                    <form action="{{ route('superadmin.vendor.toggle-status', $item->idvendor) }}" 
                                          method="POST" class="d-inline">
                                        @csrf
                                        <button type="submit" 
                                                class="btn btn-sm {{ $item->status_vendor == 'Aktif' ? 'btn-secondary' : 'btn-success' }}"
                                                onclick="return confirm('Ubah status vendor ini?')">
                                            <i class="fas fa-power-off"></i>
                                        </button>
                                    </form>
                                    
                                    <form action="{{ route('superadmin.vendor.destroy', $item->idvendor) }}" 
                                          method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
